...
6. **Delivers** the digest to Telegram (boundary-aware chunking, with a Markdown→plain-text fallback).
...
- **Implicit, untyped data contract** — every stage passes a bare `dict`; there is no schema, validation, or type safety at any boundary.
- **Limited extensibility** — fetchers and the single delivery channel are wired by hand in `main.py`; adding a source or channel means editing the orchestrator.
- **Documentation drift** — `README.md` references files and dependencies that do not exist in the tree (`web_fetcher.py`, Exa), while omitting ones that do (Tavily, tenacity).

This roadmap front-loads the **safety net (tests + CI + lint)**, then **correctness fixes**, then **performance, extensibility, and maintainability**, then **scalability/observability**. Each phase is independently shippable and ordered so that later work is protected by the tests added earlier.
...
**`main.py` — orchestrator.** A single `run_agent()` returns a shell-style exit code (0 ok, 1 no-articles/all-seen, 2 delivery-failed, 3 fatal). It accumulates a rich `status` dict and **always** writes `logs/last-run.json` in a `finally` block (so the cron wrapper can alert on stats). Good separation of *outcome* from *delivery*. The eval step is wrapped so it can never break the run. `--schedule` runs an immediate pass then hands off to a `BlockingScheduler`. **Observations:** all orchestration logging is via `print()`; the fetch sequence is hard-coded and serial; each fetcher is imported and called explicitly (no registry); per-stage constants (`ENRICH_TOP_N = 10`) are module-level literals.

**`config.py` — configuration.** Cleanly env-driven via `python-dotenv`. `RSS_FEEDS` is a well-structured list of dicts (per-feed `lookback_hours` override + optional title `client_filter`), and weekly newsletters correctly extend their window to 168h. **Observations:** there is **no validation** that required keys (Telegram token/chat ID) are present — missing keys surface only as a failed delivery at the end of a run; the LLM model names live in `agent/` and `eval/`, not here; `RESEARCH_COMPANY_DOMAINS` and the keyword filter are static literals.

**Fetchers.** Each returns `list[dict]` and **soft-fails to an empty list** on error, so one dead source never sinks the run — a strong design choice. RSS applies per-feed lookback + keyword filtering; ArXiv correctly uses timezone-aware datetimes and a 48h moderation window with early-exit on the sorted feed; Tavily rotates queries by `weekday()` to avoid repetition; Reddit and Twitter skip cleanly when credentials are absent. **Observations:** RSS date handling is timezone-naive (see §3, Bug #1); Twitter uses the deprecated `datetime.utcnow()` (Bug #3); all network fetches inside RSS are serial; there is **no shared fetcher interface** — each is an ad-hoc module-level function with its own signature conventions.

**`agent/dedup.py`.** `_canonical_url()` lowercases scheme/host, strips trailing slash, and drops query/fragment; `dedupe()` keeps the highest-priority source per canonical URL (ArXiv 100 → Tavily News 30) and preserves URL-less items. **Observations:** `host.lstrip("www.")` is a character-class strip, not a prefix strip (see §3, Bug #2). Dedup is URL-only — no title/near-duplicate detection.

**`agent/url_memory.py`.** A 7-day rolling JSON map `{date: [canonical_urls]}`, pruned on every write, atomic via `os.replace`. Memory is only persisted **after** summarisation succeeds, so transient LLM failures don't poison it. **Observations:** it shares `_canonical_url`, so it inherits Bug #2; storage is a single local file (single-writer, non-portable).
...
**`agent/summarizer.py`.** Central LLM factory `_create_llm()` (LiteLLM proxy preferred, direct Anthropic fallback), a shared transient-error classifier `_is_transient_llm_error`, and a `tenacity`-retried `_invoke_llm`. Langfuse tracing is attached via `propagate_attributes` when configured. **Observations:** model identifiers are string literals (`"claude-sonnet"`, `"claude-sonnet-4-6"`); `MAX_ARTICLES = 50` caps prompt size; the transient classifier matches on exception **class name strings**, which is brittle across SDK versions.

**`delivery/telegram.py`.** Splits on paragraph→line→word boundaries to avoid cutting through Markdown pairs, and retries each chunk as plain text if Markdown parsing 400s — delivery almost always succeeds. **Observations:** no retry on transient network/5xx/429; partial multi-chunk delivery can leave a half-sent digest with no rollback signal beyond `all_ok=False`; Telegram is the **only** channel and is imported directly by `main.py` (no delivery abstraction).
...
**`bin/` + scheduling.** `cron-run.sh` is genuinely battle-tested (lock, stale-PID detection, LiteLLM preflight/restart, sys-ops alerting, structured-status reporting via `jq`). **Observation:** it hard-codes absolute paths (`/Users/...observability`), and assumes `${PROJECT_DIR}/venv` plus Homebrew paths — non-portable.
...
`pytest` collects **0 tests**; there is no `tests/` directory, no `pyproject.toml`/`pytest.ini`, no CI workflow, no linter or type-checker config. The `eval/` suite is a *quality* harness needing live keys — it does not protect the deterministic core. Highly testable pure functions are entirely uncovered:
...
### 3.3 Synchronous, serial execution (performance & concurrency)
...
- The transient-error classifier matches on **exception class-name strings**, which silently breaks if an SDK renames its error types.
...
### 3.5 Limited extensibility (open/closed)

Adding a new fetcher requires: writing the module, importing it in `main.py`, adding a `counts` key, and appending a call block — four edits to the orchestrator. Adding a second delivery channel (Slack, email, webhook) means editing `main.py` and the `status` shape directly, because `delivery/telegram.py` is imported by name. There is **no `Fetcher` protocol, no registry, and no `DeliveryChannel` abstraction**. **Impact:** the orchestrator grows linearly with sources/channels; sources cannot be toggled by config; fan-out delivery is not possible without surgery.

### 3.6 Deduplication is URL-only (content quality)
...
### 3.7 Rate limiting & API-usage optimisation
...
### 3.8 State persistence is local-file-only
...
### 3.9 Configuration robustness & validation

`config.py` reads env vars with `os.getenv` and inline `int(...)` casts (`DAILY_HOUR`, `LOOKBACK_HOURS`). There is **no fail-fast validation**: a missing `TELEGRAM_BOT_TOKEN` is not detected until delivery; a non-numeric `DAILY_HOUR` raises a raw `ValueError` at import time with no friendly message; there is no check that *at least one* LLM backend (LiteLLM or Anthropic) is configured. **Impact:** misconfiguration fails late and opaquely instead of at startup.

### 3.10 Type safety & the untyped data contract

The inter-stage article `dict` has no declared shape. Function signatures use `list[dict]`, so static tools cannot catch a renamed key or a missing field; every access is a defensive `.get(...)`. The `status` dict grows keys ad-hoc (`filtered_by_memory`, `eval_*`), and the `jq` consumer in `cron-run.sh` is coupled to those undocumented field names. There is no `mypy` gate. **Impact:** typos and contract drift surface only at runtime.

### 3.11 Hard-coded configuration & machine coupling

- **Model names** as string literals across modules: `"claude-sonnet"` / `"claude-sonnet-4-6"` (`summarizer`), `"claude-haiku-4-5"` (`daily_eval`, `run_eval`).
...
- **`bin/cron-run.sh`** hard-codes an absolute observability path, `${PROJECT_DIR}/venv`, and Homebrew paths. **Impact:** the repo only runs unmodified on its author's machine.

### 3.12 Observability: `print()` instead of structured logging
...
### 3.13 Documentation drift (`README.md`)
...
- Source lists are inconsistent with `config.RSS_FEEDS`. **Impact:** a new contributor following the README cannot reproduce the working setup.
...
- Add `pyproject.toml` (or `pytest.ini`) with `pytest` + coverage config; set an initial coverage floor (e.g. 70%) and ratchet up.
...
- Harden `_is_transient_llm_error` to match on exception **types**/`isinstance` where possible rather than class-name strings.
...
### 4.5 Make fetchers and delivery pluggable (extensibility)
- Define a `Fetcher` `Protocol` (`name: str`, `fetch() -> list[Article]`) and register sources in a list/registry that `main.py` iterates — adding a source becomes "drop a module + register", with no orchestrator edits and a config flag to enable/disable.
- Define a `DeliveryChannel` `Protocol` (`send(digest) -> bool`) with `TelegramChannel` as the first implementation; the orchestrator fans out to all configured channels and records per-channel status. This unlocks Slack/email/webhook without touching core logic.

### 4.6 Add near-duplicate detection
...
### 4.9 Configuration robustness
- Adopt `pydantic-settings` (or a small explicit `validate_config()` called at the top of `run_agent()`) that fails fast with clear messages on missing required keys, validates ranges (`0 ≤ DAILY_HOUR ≤ 23`), and asserts at least one LLM backend is configured.
...
- Replace machine-specific literals in `bin/cron-run.sh` with env vars (`OBSERVABILITY_DIR`, `PYTHON_BIN`) sourced from `.env`, with sensible defaults.

### 4.10 Type safety & typed contracts
...
- Enable `mypy` in CI (per §4.2) so contract drift is caught statically.
...
### 4.12 Documentation realignment
- Rewrite the README to match reality: Tavily (not Exa), the actual `requirements.txt`, the `bin/cron-run.sh` cron flow, and the true module list. Add an architecture diagram and a "running tests" section. (Per constraints, this roadmap only *proposes* the rewrite; it does not edit `README.md`.)
...
- [ ] Route ranking through retried/traced `_invoke_llm`; add Telegram transient-retry with `Retry-After`; harden the transient-error classifier.
...
### Phase 5 — Extensibility: fetcher & delivery abstractions *(1–2 days)*
- [ ] Introduce the `Fetcher` protocol + registry; migrate the five fetchers behind it; drive the orchestrator loop from the registry with config-driven enable/disable.
- [ ] Introduce the `DeliveryChannel` protocol with `TelegramChannel`; make the orchestrator fan out to configured channels with per-channel status.
- **Exit criteria:** adding a source/channel needs no edit to `run_agent()`; existing behaviour unchanged (verified by tests).

### Phase 6 — Content quality: near-duplicate dedup *(1 day)*
...
### Phase 7 — Configuration & persistence *(1–2 days)*
- [ ] Add fail-fast config validation (`pydantic-settings` or `validate_config()`); centralise models/constants in `config.py` (env-overridable).
...
- **Exit criteria:** misconfiguration fails fast with clear messages; no hard-coded model names or host paths; persistence backend is swappable behind one interface.

### Phase 8 — Observability & typed contracts *(1 day)*
- [ ] Replace `print()` with `logging` + per-run correlation ID; `LOG_LEVEL` env.
- [ ] Introduce a typed `Article` model and a `TypedDict`/model for `status`; enable `mypy` gate.
- **Exit criteria:** logs carry levels + run ID; article/status schemas are validated and documented.

### Phase 9 — Documentation *(½ day)*
...
`Phase 0 → 1 → 2` form the critical path (safety net + correctness). Phases 3–8 can be parallelised across contributors once the test net exists. Phase 9 should trail whatever lands to avoid re-drift.
...
| 5 | Fetcher/delivery abstractions | 1–2 d | Med | Easy new sources & channels |
| 6 | Near-dup dedup | 1 d | Med | Higher digest quality |
| 7 | Config + persistence | 1–2 d | Med | Fail-fast config + scalability |
| 8 | Logging + typed models | 1 d | Low | Debuggability + type safety |
| 9 | Docs | ½ d | Low | Onboarding + accuracy |
...
*Document generated from static analysis of the repository at the current branch state. This plan is advisory and additive — it proposes changes without modifying any existing source files.*
