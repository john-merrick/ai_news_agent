...
# Kimi (Moonshot AI) blog (https://www.kimi.com/blog/).
# Kimi does not publish an official RSS feed, so announcements are scraped
# directly from the server-rendered blog index using stdlib parsing (no extra
# deps), mirroring the Anthropic/OpenAI fetchers. Each card carries a
# day-granular "YYYY/MM/DD" date, so use a wider default window than the global
# 24h cutoff to avoid dropping announcements posted late the prior day.
# NOTE: unlike the graceful Anthropic/OpenAI sources, the Kimi fetcher fails
# loudly (raises) on network/HTTP/parse errors — see fetchers/kimi_fetcher.py.
KIMI_NEWS_URL = os.getenv("KIMI_NEWS_URL", "https://www.kimi.com/blog/")
KIMI_LOOKBACK_HOURS = int(os.getenv("KIMI_LOOKBACK_HOURS", str(max(LOOKBACK_HOURS, 48))))
